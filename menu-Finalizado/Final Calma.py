...
app.iconbitmap("imgs/calma_airlines_logo.ico")

# Imagens
imagem_casinha = ctk.CTkImage(light_image=Image.open("imgs/passaro.png"), size=(80, 80))
img = Image.open("imgs/Calma_aviao.png").convert("RGBA")
...
def tela_cancelamento():
    ...
    canvas = tk.Canvas(frame_scroll_ext, width=540, height=200, borderwidth=0, bg="#191a20", highlightthickness=0)
    ...
    label_msg.grid(row=4, column=0, padx=20, pady=(13, 4), sticky="w")
    ...
    def buscar_voos():
        ...
        for voo in resultado:
            ...
            cb = ctk.CTkCheckBox(frame_voos, text=texto, variable=var)  # Removidos os argumentos não suportados
            cb.pack(anchor="w", pady=3, padx=2)

        frame_scroll_ext.grid()
        label_msg.configure(text="Selecione os voos para cancelar e clique em 'Cancelar voo'.", text_color="white")
        ultimo_cpf["cpf"] = cpf
        ultimo_cpf["voos"] = resultado

    def cancelar_voos_selecionados():
        cpf = ultimo_cpf["cpf"]
        if not cpf:
            label_msg.configure(text="Você precisa buscar voos primeiro.", text_color="red")
            return

        indices_para_cancelar = [idx for idx, var in checkbox_vars.items() if var.get()]
        if not indices_para_cancelar:
            label_msg.configure(text="Selecione pelo menos um voo para cancelar.", text_color="red")
            return

        mensagens = []
        for idx in sorted(indices_para_cancelar, reverse=True):  # reverse para não bagunçar índices ao remover
        ...
        label_msg.configure(text="\n".join(mensagens), text_color="green")
    ...
    botao_cancelar = ctk.CTkButton(container_conteudo, text="Cancelar voo(s)", command=cancelar_voos_selecionados)
    botao_cancelar.grid(row=5, column=0, padx=20, pady=(10, 10), sticky="w")
    botao_cancelar.grid_remove()

    def atualiza_botao_cancelar_visibilidade(*args):
        selected = any(var.get() for var in checkbox_vars.values())
        if selected:
            botao_cancelar.grid()
        else:
            botao_cancelar.grid_remove()

    # Monitorar mudanças nas checkboxes para mostrar/esconder botão cancelar
    def monitorar_checkboxes():
        for var in checkbox_vars.values():
            var.trace_add('write', lambda *args: atualiza_botao_cancelar_visibilidade())
        atualiza_botao_cancelar_visibilidade()

    # Chamar monitoramento após buscar voos (atrelado dentro buscar_voos)
    def buscar_voos_monitorado():
        buscar_voos()
        monitorar_checkboxes()

    botao_buscar.configure(command=buscar_voos_monitorado)
